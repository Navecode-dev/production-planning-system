...
@section('title', 'Riwayat Perhitungan ELECTRE Toko')
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Riwayat Perhitungan Peringkat Toko</h3>
            </div>
            <div class="card-body">
                <form method="GET" action="{{ route('electre.store.history') }}" class="mb-4">
                    <div class="row">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4 d-flex align-items-end">
                            <div class="form-group">
                                ...
                                <a href="{{ route('electre.store.history') }}" class="btn btn-secondary">Reset</a>
                                <a href="{{ route('reports.electre.store.history.pdf', request()->query()) }}" class="btn btn-danger" target="_blank">
...
$(document).ready(function() {
    $('#detailModal').on('show.bs.modal', function (event) {
        ...
        var sortedRanking = Object.entries(rankingData).sort((a, b) => b[1] - a[1]);

        var tableHtml = '<table class="table table-bordered table-hover">';
        tableHtml += '<thead class="thead-light"><tr><th>Peringkat</th><th>Toko</th><th>Skor</th></tr></thead>';
        tableHtml += '<tbody>';

        if (sortedRanking.length > 0) {
            sortedRanking.forEach(function(item, index) {
                var storeName = item[0];
                var score = item[1];
                tableHtml += '<tr><td>' + (index + 1) + '</td><td>' + storeName + '</td><td>' + score + '</td></tr>';
            });
        } else {
